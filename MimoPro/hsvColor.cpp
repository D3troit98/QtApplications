...
hsvColor::hsvColor(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::hsvColor)
{
    ui->setupUi(this);
    ...
    moveWindow(hsvWindow, 700,0);
    moveWindow(thresholdWindow, 700,500);
}

void hsvColor::closeEvent(QCloseEvent *event)
{
    breakLoop=true;
    destroyAllWindows();
...
string hsvColor::numberToString(int number){
...
void hsvColor::drawObject(int x, int y, Mat &frame, item tempItem){
...
void hsvColor::morphObject(Mat &thresh){
...
void hsvColor::trackObject(int &x, int &y, item tempItem, Mat &cameraFeed){
...
hsvColor::item hsvColor::setUpObject(string name, int hmin, int hmax, int smin, int smax, int vmin, int vmax){
...
void hsvColor::on_startbtn_clicked()
{
    Start();
}
